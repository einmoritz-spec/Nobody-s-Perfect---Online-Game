...
import { BrainCircuit, CheckCircle2, HelpCircle, Dice5, BookOpen, Gavel, Sparkles, Globe, Megaphone, Briefcase } from 'lucide-react';
...
export const GameMasterInput: React.FC<GameMasterInputProps> = ({ gameMaster, onSubmit, isHost }) => {
  ...
  const pickRandom = () => {
    const pool = QUESTIONS[category] || QUESTIONS.words;
    ...
    <div className="max-w-xl mx-auto animate-fade-in-up">
      <div className="bg-brand-accent/20 border border-brand-accent/30 rounded-lg p-4 mb-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-300">Wähle eine Kategorie oder gib eine eigene Frage ein.</p>
        </div>
      </div>

      <Card title="Die Runde vorbereiten">
        <div className="mb-6 space-y-4">
          ...
          {/* Responsive Grid für 6 Kategorien: Mobile 2 Spalten, ab Tablet 3, ab Desktop 6 */}
          ...
          <div className="flex justify-center">
             <Button type="button" onClick={pickRandom} variant="secondary" className="text-sm py-2 px-4">
              <Dice5 size={18} className="mr-2 inline" /> Zufällige Frage ({CATEGORIES.find(c => c.id === category)?.name})
            </Button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder="Stell eine Frage oder nenne ein Gesetz..."
